...
void kill_screen(const char* lcd_msg) {
  ...
  inline void line_to_z(const float &z) {
  ...
  #if ENABLED(MENU_ITEM_CASE_FAN)

    extern uint8_t case_fan_speed;
    extern bool case_fan_on;
    extern void update_case_fan();
    ...
    void case_fan_menu() {
      ...
      //
      MENU_BACK(MSG_MAIN);
      MENU_ITEM_EDIT_CALLBACK(int8, MSG_CASE_FAN_SPEED, &case_fan_speed, 0, 255, update_case_fan, true);
      MENU_ITEM_EDIT_CALLBACK(bool, MSG_CASE_FAN, (bool*)&case_fan_on, update_case_fan);
      END_MENU();
    }
  #endif // MENU_ITEM_CASE_FAN
  ...
  void lcd_main_menu() {
    ...
    //
    // Set Case fan on/off/speed
    //
    #if ENABLED(MENU_ITEM_CASE_FAN)
      if (USEABLE_HARDWARE_PWM(CASE_FAN_PIN)) {
        MENU_ITEM(submenu, MSG_CASE_FAN, case_fan_menu);
      }
      else
        MENU_ITEM_EDIT_CALLBACK(bool, MSG_CASE_FAN, (bool*)&case_fan_on, update_case_fan);
    #endif
  ...
  void lcd_prepare_menu() {
    ...
    //
      ...
      MENU_ITEM(gcode, MSG_LEVEL_GANTRY, PSTR("G28\nG34"));
    #endif
